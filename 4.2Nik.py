...
def selection(generation):
    ...
    selec_individuals = [generation['Individuals'][-x-1]
        for x in range(int(len(generation['Individuals'])//5))]
    selec_fitnesses = [generation['Fitness'][-x-1]
        for x in range(int(len(generation['Individuals'])//5))]
    selec = {'Individuals': selec_individuals,
                    'Fitness': selec_fitnesses}
    return selec
...
def mating(parents):
    ...
    offspr = []
    offspr.append(offspr1)
    offspr.append(offspr2)
    return offspr
...
def next_generation(gen):
    ...
    offspr2 = [offsprings[x][1]
                   for x in range(len(parents))]
    unmutated = offspr1.extend(offspr2)
    unmutated = offspr1
    ...
    sorted_next_gen = sorted([[unsorted_individuals[x], unsorted_next_gen[x]] for x in range(len(unsorted_individuals))], 
                key=lambda x: x[1])
...
# сходимость (функция приспособленности лучшей особи в популяциях  отличается не более, чем на стоимость самой дешевой вещи) или прошло 500 поколений
# другое решение, при первой итерации находится особь с хорошим приспособлением - алгоритм оканчивается,
# когда разница между функциями приспособленности больше, чем стоимость самой дешевой вещи
